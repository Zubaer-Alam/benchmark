...
const arcsData = countriesData.map((startCountry, i) => {
  const endCountry = countriesData[(i + 1) % countriesData.length];
  // This will create a loop between countries
  return {
    startLat: startCountry.lat,
    startLng: startCountry.lng,
    endLat: endCountry.lat,
...
const EarthComponent = () => {
  ...
  const globeReady = () => {
    if (globeRef.current) {
      globeRef.current.controls().autoRotate = true;
      globeRef.current.controls().autoRotateSpeed = 1.0; // Increase the rotation speed
      globeRef.current.controls().enableZoom = false;
      globeRef.current.pointOfView({
        lat: 19.054339351561637,
        lng: -50.421161072148465,
  ...
  return (
    <div className="">
      <Globe
        ...
        enableTouchRotate={false} // Disable touch interaction
      />
    </div>
  );
};
